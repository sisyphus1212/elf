...
main(int argc, char *argv[])
{
    ...
    /* relocate functions: puts & exit */

    /* load libs specified in .dynamic, with default path in /etc/ld.so.conf: /lib32/libc.so.6 */

    /* list all functions need to be relocated in .dynsym */

    /* get the positions to store function address .rel.plt and calculate memory address */

    /* search the functions in .symtab of libc.so, get the address and write them back to above positions */
